...
public class OrderSQL {
   public final static String ORDER_DELETE_BY_USERID = "delete from orders where u_id=?";
   public final static String ORDER_DELETE_BY_O_NO = "delete from orders where o_no=?";
   public final static String ORDER_INSERT = "insert into orders(o_no,o_desc,o_date,o_price,o_address,u_id) values (orders_o_no_SEQ.nextval,?,sysdate,?,?,?)";
   ...
   public final static String ORDER_SELECT_BY_USERID = "select * from orders where u_id=?";
   ...
   public final static String ORDER_SELECT_WITH_ORDERITEM_BY_USERID =
         "select * from orders o join order_item oi on o.o_no = oi.o_no  join  product p on oi.p_no = p.p_no where o.u_id = ?";
}